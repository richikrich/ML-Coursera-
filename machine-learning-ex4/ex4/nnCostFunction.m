...
J=-sum(sum(y_new.*log(z3))+sum((1-y_new).*log(1-z3)))/m; %unregularized

Theta1_new=Theta1(:,2:end);
Theta2_new=Theta2(:,2:end);
r=(lambda/(2*m))*(sum(sum(Theta1_new.^2))+sum(sum(Theta2_new.^2)));
J=J+r; %regularized
...
Theta1_grad(:, 2:end) = Theta1_grad(:, 2:end) + ((lambda / m) * Theta1_new);
Theta2_grad(:, 2:end) = Theta2_grad(:, 2:end) + ((lambda / m) * Theta2_new);
%}
for t = 1:m
	% Step 1 - Input Values
	...
	d2 = (Theta2_new' * d3) .* sigmoidGradient(z2);
